...
def market_long(symbol, quantity, position_side='BOTH'):  # 市价开多，添加position_side参数
    """执行币安期货市价开多操作
    
    参数:
        symbol: 交易对，如 'ETHUSDT'
        quantity: 买入数量
        position_side: 仓位方向，'BOTH'(单向持仓) 或 'LONG'(双向持仓)
    """
    url = 'https://fapi.binance.com/fapi/v1/order'
    # 当前时间戳（用于API请求，API需要UTC时间戳）
    timestamp = int(round(time.time() * 1000))
    query_string = 'timestamp=%s&symbol=%s&side=BUY&type=MARKET&quantity=%s&positionSide=%s' % (str(timestamp), symbol, str(quantity), position_side)
